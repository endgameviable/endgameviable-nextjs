...
export default async function Page({ params }: { params: { source: string } }) {
  ...
  const content =
    'List of the most recent content from the source "' +
    params.source +
    '". Sources can be things like blog posts from different categories (gaming, music, reviews, etc.) or a data sources such as a movie or book database. There might be filters and search.';
  ...
  return (
    <main>
      <EntryListLayout content={content} list={entries.slice(0, PAGE_SIZE)} />
    </main>
  );
}
