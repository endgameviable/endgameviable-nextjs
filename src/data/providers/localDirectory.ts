...
export default class ContentDirectoryProvider implements EntryProvider {
  ...
  async getEntry(route: ContentFile): Promise<Entry> {
    if (this.transformer) {
      try {
        ...
        transformed.filter(
          (entry) => entry.route == path.join(route.path, route.name),
        );
  ...
  async queryEntries(filter: EntryQueryParams): Promise<Entry[]> {
    ...
    for (const file of files) {
      ...
      if (this.transformer) {
        try {
          ...
          entries.push(
            ...transformed.filter((entry) => entryMatchesFilter(entry, filter)),
          );
        } catch (error) {
          console.log('error decoding file:', error);
        }
    ...
    console.log(
      `files scanned: ${numFiles}, entries scanned: ${numEntries}, entries returned: ${
        entries.length
      }, from: ${this.directoryPath}, files scanned in: ${scanElapsed.toFixed(
        2,
      )}ms, files transformed in: ${transformElapsed.toFixed(
        2,
      )}ms, total processed in: ${totalElapsed.toFixed(2)}ms`,
    );
    return entries;
  }
}
